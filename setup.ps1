...
try {
    ...
    if ($mysqlFound) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✔ Databases ensured successfully." -ForegroundColor Green
        } else {
            Write-Host "  ⚠ Databases might not have been created. Check if MySQL is running." -ForegroundColor Yellow
        }
    } else {
        Write-Host "  ⚠ 'mysql' CLI not found. Please ensure MySQL is installed and added to PATH." -ForegroundColor Red
    }
} catch {
    Write-Host "  ⚠ Error during DB creation: $_" -ForegroundColor Red
}
...
Write-Host "   Setup Completed Succesfully! " -ForegroundColor Green
